...
## Course Data Pipeline (catalog = source of truth)

Course titles come from the catalog, and a course number has exactly one name. The pipeline is documented in `docs/INGESTION.md`; in brief:

1. `node scripts/fetch-dept-courses.mjs` scrapes `catalog.luc.edu/course-descriptions/<dept>/` for every department in its `DEPARTMENTS` list, writing per-department files to `src/data/dept-courses/<DEPT>.json` and rebuilding the flat master map `src/data/course-index.json` (`code → {title, credits}`).
2. `node scripts/normalize-program-courses.mjs` rewrites every program JSON's course **titles** to match `course-index.json` (titles only; credits are left alone — see `CREDITS_DISCREPANCIES.md`). It reports any code not found in the catalog.
3. The `course integrity` test (`src/utils/courseIntegrity.test.ts`) fails the build if any program course code is missing from the index or its title differs. Documented exceptions live in `src/data/course-exceptions.json`; suspect codes are tracked in `LIKELY_WRONG.md`.

Never hand-type or copy a course title between program files — run the normalizer. When updating program data or rolling over to a new catalog year, follow `docs/INGESTION.md`.

## Program Data Pipeline (programs are generated, not hand-edited)

Academic programs (`src/data/<id>.json`) are reproducible from committed inputs, not authored by hand. Full details in `docs/INGESTION.md`; the layers are:

- `scripts/program-manifest.mjs` — the seed list of programs (id, catalogUrl, degree, department, kind, credit overrides). An entry with `generated: true` is **adopted** and drift-guarded.
- `scripts/fetch-program.mjs` — deterministic scrape of each catalog program page → `src/data/program-extracts/<id>.json` (courses, elective pools, roadmap; titles/credits reconciled to `course-index.json`).
- `src/data/program-refine/<id>.json` — committed AI overlay for fuzzy requirement semantics (choose-N, credit caps, choiceNote).
- `src/data/program-supplements/<id>.json` — committed non-catalog data (AP/transfer/milestone checklist, notes, credit pins).
- `scripts/build-programs.mjs` — the only writer of `src/data/<id>.json`; deep-merges the layers in a fixed, byte-stable order. `--out <dir>` for scratch diffs, `--check` for the drift guard.

`src/utils/programIntegrity.test.ts` checks manifest↔registry sync, roadmap-ref resolution, and credit sanity; `npm run check:programs` enforces byte-reproducibility of every `generated` program. **Do not hand-edit an adopted (`generated: true`) `src/data/<id>.json`** — change the manifest/extract/overlay/supplement and rebuild. Programs without the flag are legacy hand files being migrated wave by wave.
